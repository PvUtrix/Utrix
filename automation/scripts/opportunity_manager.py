...
Automated tracking, evaluation, and management of career opportunities.
"""
...
class OpportunityManager:
    def __init__(self, base_path: str = None):
        ...
        if base_path is None:
            self.base_path = Path(__file__).parent.parent.parent / "domains" / "career" / "opportunities"
        else:
    ...
    def create_opportunity(self, name: str, opportunity_type: str, source: str, 
        ...
        """Create a new opportunity folder and files."""
    ...
    def evaluate_opportunity(self, opportunity_id: str, scores: Dict[str, int]) -> Dict[str, any]:
        """Evaluate an opportunity and calculate total score."""
        opportunity_path = self.active_path / opportunity_id
        ...
        timing_score = (
            ...
            scores.get("opportunity_cost", 0) +
            scores.get("market_timing", 0) +
            scores.get("life_stage_fit", 0)
        )
        
        total_score = goals_score + values_score + timing_score
    ...
    def get_opportunities_summary(self) -> List[Dict]:
        """Get summary of all active opportunities."""
        opportunities = []
    ...
    def archive_opportunity(self, opportunity_id: str, status: str, notes: str = ""):
        """Archive an opportunity with final status."""
        opportunity_path = self.active_path / opportunity_id
        ...
        print(f"Archived opportunity {opportunity_id} with status: {status}")
...
---
*Created by Opportunity Manager on {datetime.now().strftime("%Y-%m-%d %H:%M")}*
"""
...
"""
    ...
    def _update_evaluation_file(self, opportunity_path: Path, scores: Dict[str, int], total_score: int):
        ...
        content = content.replace("**Goals Score**: ___/40", f"**Goals Score**: {sum([scores.get(k, 0) for k in ['career_growth', 'financial_impact', 'learning_opportunity', 'network_expansion']])}/40")
        ...
        content = content.replace("**Values Score**: ___/35", f"**Values Score**: {sum([scores.get(k, 0) for k in ['fairness', 'freedom', 'sustainability']])}/35")
        
        content = content.replace("**Current Capacity** (1-10): ___", f"**Current Capacity** (1-10): {scores.get('current_capacity', 0)}")
        content = content.replace("**Opportunity Cost** (1-10): ___", f"**Opportunity Cost** (1-10): {scores.get('opportunity_cost', 0)}")
        ...
        content = content.replace("**Timing Score**: ___/25", f"**Timing Score**: {sum([scores.get(k, 0) for k in ['current_capacity', 'opportunity_cost', 'market_timing', 'life_stage_fit']])}/25")
...
### Score Breakdown
- Goals Alignment: {sum([scores.get(k, 0) for k in ['career_growth', 'financial_impact', 'learning_opportunity', 'network_expansion']])}/40
- Values Alignment: {sum([scores.get(k, 0) for k in ['fairness', 'freedom', 'sustainability']])}/35
- Timing & Logistics: {sum([scores.get(k, 0) for k in ['current_capacity', 'opportunity_cost', 'market_timing', 'life_stage_fit']])}/25
...
def main():
    """Main CLI interface."""
    parser = argparse.ArgumentParser(description="Opportunity Management System")
    parser.add_argument("--base-path", help="Base path for opportunities")
    
    subparsers = parser.add_subparsers(dest="command", help="Available commands")
    
    # Create opportunity command
    create_parser = subparsers.add_parser("create", help="Create a new opportunity")
    create_parser.add_argument("name", help="Opportunity name")
    create_parser.add_argument("type", help="Opportunity type")
    ...
    eval_parser = subparsers.add_parser("evaluate", help="Evaluate an opportunity")
    eval_parser.add_argument("opportunity_id", help="Opportunity ID")
    ...
    archive_parser = subparsers.add_parser("archive", help="Archive an opportunity")
    archive_parser.add_argument("opportunity_id", help="Opportunity ID")
    ...
    manager = OpportunityManager(args.base_path)
